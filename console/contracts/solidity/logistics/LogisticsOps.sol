...
contract LogisticsOps {
    ...
    event LogisticsDelegateCreated(
        string indexed voucherNo,
        address indexed operator,
        uint256 timestamp
    );
    ...
    /**
     * @dev 创建物流委派单（运维代理接口，数组参数解决Stack too deep）
    ...
    function createLogisticsDelegate(
        ...
        require(bytes(stringParams[0]).length > 0, "Invalid voucherNo");
        require(bytesParams[0] != bytes32(0), "Invalid owner hash");

        ILogisticsCore(logisticsCore).createLogisticsDelegate(
            stringParams,
            uintParams,
            bytesParams
        );

        emit LogisticsDelegateCreated(stringParams[0], msg.sender, block.timestamp);
        return true;
    }
