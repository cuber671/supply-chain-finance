...
contract LogisticsCoreSimplified {
    ...
    function createLogisticsDelegate(
        CreateDelegateParams calldata p
    ) external onlyAdmin returns (bool success) {
        require(!voucherNoExists[p.voucherNo], "Voucher exists");
        voucherNoExists[p.voucherNo] = true;
        voucherStatus[p.voucherNo] = 1;
        delegateCount++;
        emit LogisticsDelegateCreated(p.voucherNo, msg.sender, block.timestamp);
        return true;
    }
    ...
    function arriveAndCreateReceipt(
        CreateReceiptParams calldata p
    ) external returns (bool success) {
        require(voucherNoExists[p.voucherNo], "Voucher not found");
        voucherStatus[p.voucherNo] = 4;
        emit StatusUpdated(p.voucherNo, 3, 4, msg.sender, block.timestamp);
        return true;
    }
