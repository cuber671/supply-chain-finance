...
import "../warehouse/IWarehouseReceiptCore.sol";
...
contract ReceivableRepayment {
    ...
    // ReceiptStatus.Pledged == 6 (see WarehouseReceiptCore.sol enum)
    uint8 constant RECEIPT_STATUS_PLEDGED = 6;

    modifier onlyPledgedReceipt(string calldata receiptId) {
        require(warehouseReceiptCore != address(0), "Warehouse receipt core not set");
        // 使用 getReceipt 检查仓单状态（兼容已部署的 WarehouseReceiptCore）
        (
            , , , , , ,
            uint8 status
        ) = IWarehouseReceiptCore(warehouseReceiptCore).getReceipt(receiptId);
        require(status == RECEIPT_STATUS_PLEDGED, "Receipt not pledged");
        _;
    }
    ...
    /**
     * @dev 设置仓单核心合约地址
     * @param _warehouseReceiptCore 仓单核心合约地址
     */
    function setWarehouseReceiptCore(address _warehouseReceiptCore) external onlyAdmin {
        require(_warehouseReceiptCore != address(0), "Invalid address");
     ...
     * @return success 是否成功
     */
    function offsetDebtWithWarehouseReceipt(
        string calldata sourceReceivableId,
        ...
        external onlyValidReceivableId(sourceReceivableId)
                onlyPledgedReceipt(receiptId) returns (bool success)
    {
        require(offsetAmount > 0, "Invalid amount");
        require(bytes(reason).length > 0, "Invalid reason");
        require(warehouseReceiptCore != address(0), "Warehouse receipt core not set");
